...
import { Card, Select, PageHeader, Btn, Field, Input, Grid, Divider } from '../components/UI'
...
// 高速詳細ポップアップ（PDF添付機能付き）
function HwPopup({ report, onClose, onSavePdf }: {
  report: Report
  onClose: () => void
  onSavePdf: (id: string, url: string) => void
}) {
        ...
        <div style={{ borderTop: '1px solid var(--border)', paddingTop: 14 }}>
          ...
          {pdfUrl && (
            <div style={{ marginBottom: 10 }}>
              <a href={pdfUrl} target="_blank" rel="noopener noreferrer" style={{ color: 'var(--accent)', fontSize: 12, textDecoration: 'underline' }}>🔗 添付PDFを開く</a>
            </div>
          )}
          <input ref={fileRef} type="file" accept="application/pdf" style={{ display: 'none' }} onChange={handleFile} />
          <button onClick={() => fileRef.current?.click()} disabled={uploading}
            style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '7px 14px', borderRadius: 'var(--radius)', border: '1px dashed var(--border-dark)', background: 'var(--surface2)', cursor: 'pointer', fontSize: 12, color: 'var(--text-secondary)', width: '100%', justifyContent: 'center' }}>
...
// 編集モーダル
function EditModal({ report, onClose, onSave, onDelete, prices }: {
  report: Report
  onClose: () => void
  onSave: (id: string, updates: Partial<Report>) => Promise<boolean>
  ...
  const handleCategoryChange = (v: string) => {
    setF(prev => {
      const newAmount = autoCalc ? calcAmount(v, Number(prev.crew)) : prev.amount
      return { ...prev, category: v, amount: newAmount }
    })
  }

  const handleCrewChange = (v: string) => {
    setF(prev => {
      const crew = parseInt(v) || 0
      const newAmount = autoCalc ? calcAmount(prev.category, crew) : prev.amount
      return { ...prev, crew, amount: newAmount }
    })
  }

  const handleDelete = async () => {
    setDeleting(true)
    const ok = await onDelete(report.id)
    setDeleting(false)
    if (ok) onClose()
  }
  ...
  const handleSave = async () => {
    setSaving(true)
    const exp = (parseInt(String(f.park_fee)) || 0) + (parseInt(String(f.hw_fee)) || 0) + (parseInt(String(f.meal)) || 0) + (parseInt(String(f.other_exp)) || 0)
    const ok = await onSave(report.id, { ...f, expenses: exp })
    setSaving(false)
    if (ok) onClose()
  }
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.5)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }}
      ...
      <div style={{ background: 'var(--surface)', borderRadius: 12, padding: '20px 24px', width: '100%', maxWidth: 680, maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 8px 32px rgba(0,0,0,.18)' }}>
        ...
        <Grid cols={3} style={{ marginBottom: 10 }}>
          ...
          <Field label="対応区分">
            <select value={f.category} onChange={e => handleCategoryChange(e.target.value)}
              style={{ padding: '6px 9px', border: '1px solid var(--border-dark)', borderRadius: 'var(--radius)', background: 'var(--surface)', color: 'var(--text)', fontSize: 13, width: '100%' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
          <label style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, cursor: 'pointer', color: 'var(--text-secondary)' }}>
            <input type="checkbox" checked={autoCalc} onChange={e => {
              ...
              if (e.target.checked) setF(prev => ({ ...prev, amount: calcAmount(prev.category, Number(prev.crew)) }))
            }} />
            対応区分・船員人数から自動計算
          </label>
          {autoCalc && (
            <span style={{ fontSize: 11, color: 'var(--accent)', background: 'var(--accent-bg)', padding: '2px 8px', borderRadius: 'var(--radius)', border: '1px solid var(--accent-border)' }}>
              自動計算中 ¥{calcAmount(f.category, Number(f.crew)).toLocaleString()}
            </span>
          )}
        </div>
        <Grid cols={2} style={{ marginBottom: 10 }}>
          <Field label="売上金額（円）">
            <Input type="number" value={String(f.amount)} onChange={v => { setAutoCalc(false); setNum('amount')(v) }} />
          </Field>
        ...
        <div style={{ display: 'flex', gap: 10, justifyContent: 'space-between', marginTop: 16 }}>
          <div>
            {!confirmDelete
              ? (
                <Btn onClick={() => setConfirmDelete(true)} disabled={deleting}>
                  🗑 削除
                </Btn>
              )
              : (
                <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                  ...
                  <Btn onClick={handleDelete} disabled={deleting}>
...
function HwCell({ report, onClick }: { report: Report; onClick: () => void }) {
  if (report.hw_fee === 0 && !report.hw_from1 && !report.hw_from2) {
    return <span style={{ color: 'var(--text-light)', fontSize: 11 }}>—</span>
  }
  return (
    <div onClick={onClick} title="クリックして高速詳細を表示"
      style={{ cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 3, padding: '2px 6px', borderRadius: 'var(--radius)', border: '1px solid var(--border)', background: 'var(--surface2)', transition: 'all .12s', fontSize: 11 }}
...
export default function ReportsList({ reports, onUpdateAmount, onSavePdf, onUpdateReport, onDeleteReport, prices = {} }: Props) {
  ...
  const months = [...new Set(
    reports.filter(r => !filterYear || r.bill_month?.startsWith(filterYear)).map(r => r.bill_month)
  )].sort().reverse()
  ...
  const handleUpdateReport = async (id: string, updates: Partial<Report>): Promise<boolean> => {
    const ok = await onUpdateReport(id, updates)
    return ok
  }

  return (
    <div style={{ padding: '16px 18px' }}>
      <PageHeader title="日報一覧" sub="行をクリック→編集 / 高速料金をクリック→詳細 / 売上金額をクリック→編集">
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 12, flexWrap: 'wrap' }}>
        ...
        {zeroCount > 0 && (
          <div style={{ background: 'var(--warning-bg)', border: '1px solid var(--warning-border)', borderRadius: 'var(--radius)', padding: '8px 12px', fontSize: 12, color: 'var(--warning)' }}>
            ⚠ 売上未入力: <strong>{zeroCount}件</strong>
          </div>
        )}
      </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              ...
              <tr style={{ background: 'var(--surface2)' }}>
                {COLS.map(([h, w]) => (
                  <th key={h} style={{
                    ...
                    background: h === '売上金額' ? 'var(--accent-bg)' : undefined,
                  }}>{h}</th>
                ))}
              </tr>
            </thead>
            <tbody>
              {sorted.length === 0
                ...
                : sorted.map(r => (
                  <tr key={r.id} onClick={() => setEditReport(r)}
                  ...
                  >
                    ...
                    <td style={{ ...TD, maxWidth: 150, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', color: 'var(--text-muted)' }} title={r.work}>{r.work || '—'}</td>
                    <td style={{ ...TD, fontSize: 11 }}>{r.park_fee > 0 ? `¥${r.park_fee.toLocaleString()}` : '—'}</td>
                    <td style={TD} onClick={e => { e.stopPropagation(); setHwReport(r) }}>
                      <HwCell report={r} onClick={() => setHwReport(r)} />
                    </td>
                    <td style={{ ...TD, fontSize: 11 }}>{r.meal > 0 ? `¥${r.meal.toLocaleString()}` : '—'}</td>
                    <td style={{ ...TD, fontSize: 10, maxWidth: 72, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}
                      onClick={e => e.stopPropagation()}>
                      {r.voucher
                        ? r.voucher.startsWith('http') || r.voucher.startsWith('data:')
                          ? <a href={r.voucher} target="_blank" rel="noopener noreferrer"
                              style={{ color: 'var(--accent)', textDecoration: 'underline', fontSize: 10 }}>🔗 開く</a>
